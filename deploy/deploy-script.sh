...
docker build -t conference-data:1 neo4j/initData/
...
# only needed to expose externally
#kubectl delete svc -n persistence neo4j-neo4j
#kubectl apply -f neo4j/neo-service-update.yaml
...
neo_cluster=bolt+routing://neo4j-neo4j.persistence.svc.cluster.local:7687
...
echo "using $UI_URL as ui and $API_URL as api"
