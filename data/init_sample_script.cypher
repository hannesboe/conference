...
create (:Talk {id: "TALK8", name: "Connect from Browsers Using gRPC-Web", description: "", type:"talk", commission:"free"})
create (:Talk {id: "TALK14", name: "V	Linkerd in Production 101 by Buoyant ", description: "This is a hands-on workshop that teaches how to use Linkerd in production, covering both Kubernetes and non-Kubernetes environments. This workshop is presented by Buoyant, the primary sponsors of the Linkerd project, and includes training by Linkerd maintainers.", type:"workshop", commission:"free"});
create (:Talk {id: "TALK42", name: "LF Networking Reception", description: "LF Networking, an initiative at the Linux Foundation made up of several prominent projects in the open networking stack — FD.io, ONAP, OpenDaylight, ONFV, PNDA, SNAS, and Tungsten Fabric, will be hosting a reception onsite at KubeCon + CloudNativeCon North America.", type:"talk", commission:"free"});
create (:Talk {id:"TALK61", name: "Real-time Vision Processing on Kubernetes: Working with Data Locality - Yisui Hu, Google ", description: "Kubernetes as a general purpose cloud native application platform simplifies the operation of applications. Data processing is one of the common of applications that benefit from the power of Kubernetes. However attempting real-time computer vision processing on Kubernetes isn't that straightforward, as the maximum processing time for a single frame is expected to be less than 30ms to reach 30fps or no more than 60ms (15fps, start being laggy). This presentation demonstrates the actual issues, feature gaps encountered and workarounds applied running a real-time vision pipeline on Kubernetes from the presenter's hobby robotics project. The pipeline involves a few image pre-processing tasks, a ML model (SSD mobilenet) for object detection and depth estimation with a single camera. The biggest challenge is how to distribute the tasks across nodes with data locality in consideration.", type:"talk", commission:"free"});
create (:Talk {id:"TALK75", name: "Experience Report: Running a Distributed System Across Kubernetes Clusters" , description: "Kubernetes makes it easy to run distributed applications, even those that manage persistent state, within the confines of a single cluster. Running the same applications in a multi-region or multi-cloud fashion across multiple Kubernetes clusters, however, is considerably more difficult due to the networking and service discovery problems involved.", type:"talk", commission:"free"})
create (:Talk {id:"TALK76", name: "Tutorial: Developing Production Ready Cloud Native NATS Applications", description: "Learn how to develop secure and reliable cloud native applications using NATS simple but powerful pub/sub messaging features. In this tutorial, members of the core NATS team will show how to implement from scratch a service that uses NATS with all the recommended best practices for production operation.", type:"talk", commission:"free"})
create (:Talk {id: "TALK77", name: "C	Using Istio and GKE On Prem to Manage On-Prem Data Center Downtime and Maintenance", description: "In this workshop, we will show how to combine Istio, GKE, and GKE On Prem to handle common data center operations. First, we will go through the steps of deploying Istio across environments and setting up Istio routing rules. Next we will show how you can use Kubernetes cron jobs to push traffic to the cloud for scheduled on-prem downtime and configure a “watchdog” to prioritize traffic to the cloud in the event of a traffic spike that your on-prem cannot handle.", type:"workshop", comission:"free"});

create (:Talk {id:"TALK78", name: "The Cloud Native Culture & Industry Summit by Oracle" , description: "
About the Summit 
...
create (a:Allocation { startDateTime:"2019-09-06T17:00:00:0000Z", endDateTime:"2019-09-06T18:00:00.0000Z"})
-[:HELD_IN]->(r)
create (a)-[:HAS_ASSIGNED]-> (s1)
create (a)-[:IS_ABOUT]->(t);
...
create (a:Allocation { startDateTime:"2019-09-06T15:00:00:0000Z", endDateTime:"2019-09-06T17:00:00.0000Z"})
-[:HELD_IN]->(r)
create (a)-[:HAS_ASSIGNED]-> (s1)
create (a)-[:IS_ABOUT]->(t);
